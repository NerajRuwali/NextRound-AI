...
function RevisionHub() {
  ...
  if (loading) return <div className="text-gray-300 dark:text-zinc-500 text-center py-20 animate-pulse font-mono text-sm tracking-widest uppercase">Fetching Historical Data...</div>;
    ...
    <div className="animate-in fade-in duration-300 h-full flex flex-col">
       <div className="mb-10 lg:flex items-end justify-between">
         <div>
            ...
            <p className="text-gray-300 dark:text-zinc-500">Review past questions, your responses, and system feedback to identify growth areas.</p>
         </div>
         <div className="mt-4 lg:mt-0 font-mono text-sm text-gray-300 dark:text-zinc-500 px-3 py-1 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded uppercase tracking-widest shadow-sm dark:shadow-none transition-colors">
       ...
       {allAnswers.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-12 bg-gray-50 dark:bg-zinc-900/30 rounded-2xl border border-gray-200 dark:border-zinc-800 border-dashed text-center mt-6 transition-colors">
             ...
             <p className="text-gray-300 dark:text-zinc-500 max-w-sm">No historical responses detected. Take a mock interview to establish baseline data.</p>
          </div>
       ) : (
             ...
             {allAnswers.map((item, index) => (
                <div key={index} className="bg-white dark:bg-zinc-900/40 border border-gray-200 dark:border-zinc-800 rounded-xl overflow-hidden shadow-sm hover:border-gray-300 dark:hover:border-zinc-700 transition-colors">
                   <div className="p-5 border-b border-gray-100 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900/80 flex items-start justify-between gap-4 transition-colors">
                      <div>
                         <span className="inline-block px-2 py-0.5 rounded bg-yellow-400 dark:bg-yellow-400/10 text-yellow-400 dark:text-yellow-400 text-[10px] uppercase font-bold tracking-widest mb-3 border border-yellow-400 dark:border-yellow-400/20">
                            {item.role}
                         </span>
                      ...
                      <div className="shrink-0 flex items-center justify-center w-12 h-12 rounded-lg bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 shadow-sm dark:shadow-inner transition-colors">
                         <span className={`font-bold text-lg ${item.score >= 8 ? 'text-yellow-400 dark:text-yellow-400' : item.score < 5 ? 'text-yellow-400 dark:text-yellow-400' : 'text-yellow-400 dark:text-yellow-400'}`}>
                            {item.score}
                         </span>
                      </div>
                   </div>
