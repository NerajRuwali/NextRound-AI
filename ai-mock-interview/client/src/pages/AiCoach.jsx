    ...
    <div className="flex flex-col h-[calc(100vh-10rem)] w-full max-w-4xl mx-auto bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden transition-colors animate-in fade-in duration-300">
       ...
       <div className="h-14 border-b border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900 flex items-center px-6 transition-colors shrink-0">
          <div className="flex items-center gap-3">
             <div className="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 flex items-center justify-center transition-colors">
       ...
       <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50/50 dark:bg-zinc-950/50 transition-colors">
          {messages.map((msg, i) => (
             <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.role === 'system' && (
                   <div className="w-8 h-8 rounded-xl bg-indigo-600 flex items-center justify-center text-white self-end mb-1 mr-3 shrink-0 shadow-sm">
       ...
       <div className="p-4 bg-white dark:bg-zinc-900 border-t border-gray-200 dark:border-zinc-800 transition-colors">
          <form onSubmit={handleSend} className="relative flex items-center">
             <input
                ...
                className="w-full pl-5 pr-14 py-3.5 bg-gray-100 dark:bg-zinc-950 border border-transparent focus:border-indigo-300 dark:focus:border-indigo-500/50 focus:bg-white dark:focus:bg-zinc-900 rounded-xl text-gray-900 dark:text-zinc-100 placeholder-gray-500 dark:placeholder-zinc-600 focus:outline-none transition-all text-sm"
             />
             <button 
                type="submit"
          ...
          <div className="text-center mt-3 text-[10px] text-gray-400 dark:text-zinc-600 uppercase tracking-widest font-bold">
