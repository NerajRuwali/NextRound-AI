    ...
    <div className="animate-in fade-in duration-300 h-full flex flex-col">
       <div className="mb-10 lg:flex items-end justify-between">
         <div>
            ...
            <p className="text-gray-500 dark:text-zinc-500">Sharpen your logic and system architecture skills with targeted problems.</p>
         </div>
         <div className="mt-4 lg:mt-0 font-mono text-sm px-4 py-2 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 text-indigo-700 dark:text-indigo-400 rounded-lg flex items-center gap-2 font-bold shadow-sm">
            <span>Streak</span>
            3 Day Streak
         </div>
       </div>
       ...
       <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <input 
            ...
            className="flex-1 px-4 py-2.5 bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-800 rounded-xl text-gray-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 shadow-sm"
          />
       ...
       <div className="bg-white dark:bg-zinc-900/40 border border-gray-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm dark:shadow-none transition-colors">
         <table className="w-full text-left border-collapse">
           <thead>
             <tr className="border-b border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900/80 text-gray-500 dark:text-zinc-500 text-xs uppercase tracking-widest font-bold">
             ...
             {PROBLEM_SET.map((prob) => (
               <tr key={prob.id} className="hover:bg-gray-50 dark:hover:bg-zinc-900/50 transition-colors group">
                 ...
                 <td className="px-6 py-4">
                   ...
                   <div className="text-xs font-mono text-gray-500 dark:text-zinc-500">{prob.category}</div>
                 </td>
                 <td className="px-6 py-4 hidden sm:table-cell">
                   <span className={`text-xs font-bold uppercase tracking-widest ${prob.difficulty === 'Easy' ? 'text-emerald-500' : prob.difficulty === 'Medium' ? 'text-amber-500' : 'text-rose-500'}`}>
                     {prob.difficulty}
                   </span>
                 </td>
                 <td className="px-6 py-4 font-mono text-sm text-gray-500 dark:text-zinc-400 hidden md:table-cell">
                   {prob.acceptance}
                 </td>
                 <td className="px-6 py-4 text-right">
                   <button 
                     onClick={() => alert('Code practice IDE is a future premium feature.')}
                     className="px-4 py-2 bg-indigo-50 dark:bg-zinc-800 border border-indigo-100 dark:border-zinc-700 hover:border-indigo-300 dark:hover:border-zinc-500 hover:bg-indigo-100 dark:hover:bg-zinc-700 text-indigo-700 dark:text-zinc-300 font-medium text-sm rounded-lg transition-all"
                   >
                     Solve
                   </button>
                 </td>
